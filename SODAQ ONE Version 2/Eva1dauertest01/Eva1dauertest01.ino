...
void setup() {
  ...
  // switch for external power supply (boost converter)
  pinMode(SW1, OUTPUT); // 6
  digitalWrite(SW1, HIGH); // that is OFF
...
void loop() {
  ...
  // digitalWrite(ENABLE_PIN_IO, HIGH);
  // switch on boost converter
  // digitalWrite(SW1, LOW); // that is ON
  // delay(200);

  // do something here ...

  temperature = tempHumSensor.getCelsiusHundredths();
  humidity = tempHumSensor.getHumidityPercent();

  String packet = String(temperature / 100.0) + "/" + String(humidity);
  lightRedLED();
  sendPacket(packet);
  lightBlueLED();
  delay(50);
